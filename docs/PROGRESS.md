...
- 设置页新增“备份与迁移”入口，并进入可返回的配置导入页。
- 支持选择 `.enc` 配置文件并输入密码解密。
- 新增加密配置导入格式解析，使用 PBKDF2-HMAC-SHA256 与 AES-GCM。
- 支持导入账号配置、SecureStorage 密钥引用、应用设置、语言设置和翻译相关设置。
- 同邮箱账号冲突时可逐项选择覆盖或跳过。
- 导入后提示用户测试已导入账号连接。
- 明确不导入邮件正文、附件缓存和搜索索引。
- 增加导入解密、冲突跳过和冲突覆盖测试。
...
12. `codex/backup-import`：配置导入和冲突处理。（已完成，PR 待创建）
