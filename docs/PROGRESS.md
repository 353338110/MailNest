...
- Provider 级附件传输取消和字节级进度百分比。本轮 UI 可停止批量队列的后续下载；底层 IMAP 拉取仍是一次性 Future。
...
- 已完成：导入成功后提供“测试连接”入口，直接打开导入账号编辑/测试页。
- 已完成：导出同步范围设置，并兼容导入当前嵌套备份格式与旧版扁平格式。
...
- 已完成：邮件删除保留确认弹窗，单封/批量邮件操作显示运行中进度条并防止重复触发。

### 邮件操作

- 已完成：批量移动和右键移动支持目标文件夹选择，复用已同步文件夹列表。
...
2. 附件下载体验增强：大附件进度、取消、批量下载和更多真实 MIME 样本回归测试。
