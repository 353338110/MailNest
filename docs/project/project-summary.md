...
- ✅ 远端草稿箱同步（IMAP Drafts、Gmail Draft API、Microsoft Graph drafts）
- ✅ 发送或删除本地草稿后清理远端草稿

### 配置导入导出

- ✅ 导入后测试连接入口（导入成功后可直接打开账号编辑/测试页）
- ✅ 导出同步设置，并兼容当前嵌套备份格式导入
...
- ✅ 翻译结果本地缓存（按正文 hash、目标语言和 provider 配置隔离）
- ✅ 翻译缓存清理入口
- ✅ 配置备份不导出翻译缓存内容
...
- ✅ 邮件列表键盘快捷键
- ✅ 邮件列表右键菜单
- ✅ 更完整的确认和加载状态（邮件删除确认，单封/批量操作运行中进度）
- ✅ 移动端抽屉移除过期文件夹未来提示，文件夹统一在账号下展示
- ✅ 账号文件夹和展开/收起文案统一使用生成的本地化资源
...
- ✅ 附件全部下载、批量进度、成功/失败计数和停止后续下载
- ✅ 单个附件下载中状态、取消入口和当前详情页完成状态
- ✅ 中文文件名、多附件顺序定位 MIME 回归测试
- ❌ Provider 级附件传输取消和字节级进度百分比
...
2. **附件传输增强**：Provider 级附件传输取消和字节级进度百分比
...
MailNest 当前已完成第一阶段的核心功能：

- ✅ **账号管理**：多账号、多平台（IMAP/Gmail/Outlook）、OAuth 授权
- ✅ **邮件列表**：同步、列表展示、本地搜索
- ✅ **邮件详情**：正文渲染、附件下载、已读标记
- ✅ **写信与发信**：草稿、发信、发送记录
- ✅ **配置导入导出**：加密配置、安全导入
- ✅ **翻译功能**：翻译 UI 和服务接入
- ✅ **发布流程**：SemVer tag + GitHub Release
